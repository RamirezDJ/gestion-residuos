<x-admin-layout :breadcrumb="[
                ...
                <tbody>
                    @foreach ($subproductos as $subproducto)
                        <tr class="bg-white border-b">
                            ...
                            {{-- CELDA MODIFICADA: APLICAR CLASE CONDICIONAL --}}
                            <td class="px-6 py-4">
                                @php
                                    $categoriaNombre = $subproducto->categoria->nombre ?? 'Sin asignar';
                                    // Definimos la clase condicional
                                    $claseColor = $subproducto->categoria 
                                        ? 'text-gray-800' // Si tiene categoría, texto normal/oscuro
                                        : 'text-gray-400 font-semibold'; // Si NO tiene categoría, texto rojo y negrita
                                @endphp
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.subproductos.edit', $subproducto) }}">Editar</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="p-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:text-blue-400" role="alert">
            <span class="font-medium">Alerta!</span> Aún no hay subproductos registrados en el sistema.
        </div>
