        ...
        <div
            ...
            <div>
                <form action="{{ route('gensemanal.updateAll') }}" method="POST">
                    ...
                    <div class="flex justify-between content-center border-b-2 py-4 mb-4">
                        <div class="w-1/2 px-2 mb-4">
                            ...
                            <div class="relative">
                                ...
                                <input type="hidden" name="fecha_anterior" value="{{ $fecha }}">
                                <input id="datepicker-autohide" datepicker datepicker-autohide
                                    datepicker-format="dd/mm/yyyy" type="text" name='fecha'
                                    ...
                                    placeholder="Seleccionar fecha" value="{{ old('fecha', $fecha) }}">
                            </div>
                        </div>
                        <div class="w-1/2 px-2 mb-4"> <!-- Agregamos px-2 para un pequeño margen entre los divs -->
                            ...
                            <div class="relative">
                                <input type="hidden" name="turno_anterior" value="{{ $turno }}">
                                <x-select class="w-full" name="turno">
                                    <option value="" disabled>
                                        -- Seleccione un turno --
                                    </option>
                                    <option value="Matutino" {{ $turno === 'Matutino' ? 'selected' : '' }}>
                                        Matutino
                                    </option>
                                    <option value="Vespertino" {{ $turno === 'Vespertino' ? 'selected' : '' }}>
                                        Vespertino
                                    </option>
                                </x-select>
                            </div>
                    ...
                    @foreach ($registros as $zonaId => $areas)
                        <div class="text-gray-600 mb-2">
                            <p class="text-lg font-bold mb-4">
                                {{-- $zonaId --}} {{ $areas->first()->zona }}
                            </p>
                        </div>
                        <div class="grid grid-cols-4 mb-4 border-b-2 pb-4">
                            @foreach ($areas as $area)
                                <div class="mb-4 col-span-1">
                                    ...
                                    <x-input placeholder="0kg"
                                        name="valor_kg[{{ $zonaId }}][{{ $area->area_id }}]"
                                        value="{{ old('valor_kg.' . $zonaId . '.' . $area->area_id, $area->valor_kg) }}" />
                                </div>
                            @endforeach
                        </div>
                    @endforeach
                    <div class="flex justify-between items-baseline pt-4">
                        <div class="flex items-center mb-4">
                            ...
                            <label for="default-checkbox" class="ms-2 text-gray-900 dark:text-gray-300">Confirmo
                                que la informacion editada es correcta</label>
                        </div>
                        <x-button>
